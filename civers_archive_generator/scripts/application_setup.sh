...
setup_permissions() {
    ...
    if [[ ! -f "archive_generators/singlefile/single-file-x86_64-linux" ]]; then
        log_error "SingleFile binary not found at archive_generators/single-file-x86_64-linux"
        exit 1
    fi
    
    # Make SingleFile binary executable
    chmod +x archive_generators/singlefile/single-file-x86_64-linux || handle_error "Making SingleFile binary executable"
...
verify_installation() {
    ...
    if [[ ! -x "archive_generators/singlefile/single-file-x86_64-linux" ]]; then
...
print_final_instructions() {
    ...
    echo "   uv run python scripts/send_test_requests.py"
    echo ""
    echo "3. Or archive specific URLs:"
    echo "   uv run python scripts/send_test_requests.py https://example.com"
    echo ""
