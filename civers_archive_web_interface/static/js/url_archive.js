...
function urlArchive(urlId) {
    return {
        ...
        transformArtifacts(availableArtifacts) {
            // Map API artifact names to display types
            const artifactMapping = {
                'wacz': ['wacz', 'archive.wacz'],
                'singlefile': ['singlefile.html'],
                'screenshot': ['screenshot.png'], 
                'metadata': ['metadata.json']
            };
            
            return Object.keys(artifactMapping).map(displayType => ({
                type: displayType,
                exists: availableArtifacts.some(artifact => 
                    artifactMapping[displayType].some(pattern =>
                        artifact.toLowerCase().includes(pattern.toLowerCase())
                    )
                )
            }));
        },
