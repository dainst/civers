...
class EventPublisher:
    ...
    async def publish_extraction_completed(self, event: MetadataExtractionCompletedEvent) -> bool:
        ...
        try:
            ...
            topic = self.topics.get('metadata_extraction_completed') or \
                    self.topics.get('metadata_completed') or \
                    self.topics.get('metadata_extracted')
    ...
    async def publish_extraction_failed(self, event: MetadataExtractionFailedEvent) -> bool:
        ...
        try:
            # Check both internal and shared config topic keys
            topic = self.topics.get('metadata_extraction_failed') or \
                    self.topics.get('metadata_failed')
    ...
    async def _publish_event(self, topic: str, key: str, event_data: Dict[str, Any]) -> bool:
        """
        Core async event publishing method with native delivery confirmation.
        ...
        try:
            ...
            # Send the message with native async delivery confirmation
            await self.connection_manager.producer.send(
                topic=topic,
                key=key,  # Let the producer's key_serializer handle encoding
                value=event_data
            )
