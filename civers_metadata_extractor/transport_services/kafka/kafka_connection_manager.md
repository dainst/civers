...
The `KafkaConnectionManager` works with the `KafkaConfig` model defined in `configs/config_data_model.py` and uses the configuration structure from `app_config.yaml`:
