...
# Optional HTTP client for API verification
try:
    import httpx
...
class SimpleDemo:
    ...
    def __init__(self, config_path: str = "app_config.yaml", document_url: str = None, request_id: str = None):
    ...
    def _display_metadata_preview(self, json_data):
        ...
        extraction_result = json_data.get('metadata_extraction_result', {})
    ...
    async def verify_civers_upload(self, request_id: str, local_json_path: str = None):
        ...
        if not HTTPX_AVAILABLE:
            print("\n⚠️  Cannot verify CIVERS upload - httpx not available")
        ...
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                ...
                params = {
                    "snapshot_id": estimated_snapshot_id,
                    "type": "metadata.json"
                }
                ...
                if response.status_code == 200:
                    ...
                    if local_json_path and Path(local_json_path).exists():
                        ...
                        if matches:
                            print(f"✅ Files match - upload verified!")
                            return True
                        else:
                            print(f"⚠️  Files differ - possible issue")
                            return False
                    else:
                        # No local file to compare, but download successful
                        print(f"✅ Upload verified (download successful)")
                        return True
                    ...
                    print(f"   - The snapshot_id doesn't match (timing issue)")
                    print(f"   - File wasn't uploaded to CIVERS API")
                    print(f"   - Local file storage was used instead")
                    return False
                else:
    ...
    def _compare_metadata(self, local_data: dict, downloaded_data: dict) -> bool:
        ...
        local_request_id = local_data.get('metadata_extraction_result', {}).get('request_information', {}).get('request_id')
        downloaded_request_id = downloaded_data.get('metadata_extraction_result', {}).get('request_information', {}).get('request_id')
...
Examples:
  ...
  python simple_demo.py --document-url "http://127.0.0.1:8000/api/artifacts/serve?snapshot_id=req_123&type=document.html"
    ...
    parser.add_argument(
        ...
        help='Path to configuration file (default: app_config.yaml)'
    )
    
    parser.add_argument(
        '--request-id',
        type=str,
        help='Request ID to use for the demo'
    )
    ...
    if args.document_url:
        print(f"📥 Mode: Download HTML from document_url")
        print(f"   URL: {args.document_url}")
    else:
        print(f"💾 Mode: Use embedded HTML content")
    print()
    
    demo = SimpleDemo(config_path=args.config, document_url=args.document_url, request_id=args.request_id)
