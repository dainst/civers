from typing import List, Literal, Dict, Optional, Any, Union, Set, ClassVar
...
class DomainConfig(BaseModel):
    ...
    name: str
    input_source: Literal["html_document", "json_file", "xml_file", "api_response"] = "html_document"
    mappings: Dict[str, str] = Field(default_factory=dict)  # Mapping rules: source_field -> target_field
    ...
    def validate_enabled_transports_have_config(self):
        ...
        for transport in self.enabled:
            if transport not in transport_config_mapping:
                missing_configs.append(f"Transport '{transport}' is enabled but not supported. Available transports: {list(transport_config_mapping.keys())}")
            elif transport_config_mapping[transport] is None:
                missing_configs.append(f"Transport '{transport}' is enabled but no configuration provided")
...
class StorageConfig(BaseModel):
    ...
    @field_validator('enabled')
    @classmethod
    def validate_enabled_backends(cls, v: Optional[List[str]]) -> Optional[List[str]]:
        """Validate that enabled backends list is not empty if provided."""
        if v is not None and len(v) == 0:
            raise ValueError("'enabled' list cannot be empty. Provide at least one backend or use 'backend' field.")
        return v
...
class ConfigDataModel(BaseModel):
    ...
    def sync_transport_config(self) -> 'ConfigDataModel':
        ...
        if self.transport and self.app.transport:
            ...
            if not self.app.transport.kafka and self.transport.kafka:
                ...
                if not self.app.transport.kafka.bootstrap_servers and self.transport.kafka.bootstrap_servers:
                    self.app.transport.kafka.bootstrap_servers = self.transport.kafka.bootstrap_servers
    ...
    @classmethod
    def load(cls, config_dir: Optional[str] = None, environment: Optional[str] = None):
        """
        Helper to load configuration from directory.
        """
        from .loaders import YamlFileConfigLoader
        loader = YamlFileConfigLoader(config_dir=config_dir, environment=environment)
        return loader.load()
    ...
    def validate_domain_configurations(self) -> Dict[str, Any]:
        ...
        for domain in self.domains:
            try:
                ...
                if not domain.name or ('.' not in domain.name and '*' not in domain.name and domain.name != "default"):
