...
class MetadataExtractionService(MetadataExtractionServiceInterface):
    ...
    def __init__(self, config_data_model: ConfigDataModel):
        ...
        # Validate HTTP capabilities at initialization
        self._validate_http_capabilities()
        
        self.logger.info("MetadataExtractionService initialized with injected mapper and storage manager")
    ...
    async def extract_metadata(
        ...
        try:
            ...
            # Early validation for HTTP dependency if document_url is provided
            if document_url and not HTTPX_AVAILABLE:
                processing_time = time.time() - start_time
                return ExtractionResult.failure_result(
                    request_id=request_id,
                    processing_time=processing_time,
                    error_message="HTTP download requires httpx library. Install with: uv sync --extra http",
                    error_type="DependencyError",
                    failed_stage="dependency_check",
                    source_url=url
                )
            
            # Step 1: URL validation (existing logic)
            ...
            if document_url:
                ...
                try:
                    ...
                    self.logger.info(f"Successfully downloaded {len(content)} characters from document URL")
            ...
            json_output_path = await self._generate_json_output(result, mapping_result)
            if json_output_path:
            ...
            self.logger.info(f"Successfully extracted metadata from {url} in {processing_time:.2f}s")
    ...
    def _validate_http_capabilities(self):
        """Validate HTTP client availability and log status."""
        if not HTTPX_AVAILABLE:
            self.logger.warning(
                "HTTP content fetching disabled - httpx not available. "
    ...
    async def _fetch_content_from_url(self, url: str, timeout: int = 30) -> str:
        """
        ...
        """
        if not HTTPX_AVAILABLE:
            raise ContentFetchError("httpx library is required for HTTP content fetching")
        ...
        try:
            ...
            raise ContentFetchError(f"HTTP request failed: {e}")
        except httpx.HTTPStatusError as e:
            raise ContentFetchError(f"HTTP error {e.response.status_code}: {e}")
        except Exception as e:
            raise ContentFetchError(f"Unexpected error fetching content: {e}")
        ...
        try:
            ...
            output_data = {
                ...
                "intermediate_metadata": {
                    "datacite_model": result.intermediate_metadata.model_dump() if result.intermediate_metadata else None,
                    "model_validation": {
                        "is_valid": result.intermediate_metadata is not None,  # If metadata exists, validation passed
                ...
                "extraction_analytics": {
                    "total_fields_available": mapping_result.mapped_fields_count + mapping_result.skipped_fields_count,
                    ...
                    "quality_metrics": {
                        ...
                        "mapping_coverage": mapping_result.mapped_fields_count / (mapping_result.mapped_fields_count + mapping_result.skipped_fields_count) if (mapping_result.mapped_fields_count + mapping_result.skipped_fields_count) > 0 else 0.0,
                        "success_rate": 1.0 if result.success else 0.0
                    }
                },
                "system_metadata": {
                    "extraction_service_version": "2.0.0",
            ...
            storage_result: MultiStorageResult = await self.storage_manager.store_metadata(
                data=output_data,
                request_id=result.request_id,
                url=result.source_url if hasattr(result, 'source_url') else "unknown",
                filename=filename
            )
            ...
            if storage_result.overall_success:
                ...
                # All backends failed
                self.logger.error(
                    f"❌ All storage backends failed for request {result.request_id}"
                )
                return None
    ...
    def _generate_mapping_statistics(self, mapping_result: MappingResult) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_attempted": mapping_result.mapped_fields_count + mapping_result.skipped_fields_count,
            "success_rate": mapping_result.mapped_fields_count / (mapping_result.mapped_fields_count + mapping_result.skipped_fields_count) if (mapping_result.mapped_fields_count + mapping_result.skipped_fields_count) > 0 else 0.0,
