...
class HTMLParser:
    ...
    def parse_html(html_content: str) -> Any:
        ...
        try:
            ...
            try:
                ...
                raise HTMLParsingError(
                    f"Failed to parse HTML content: {e}. Fallback also failed: {fallback_e}"
                ) from fallback_e
        ...
        for script in scripts:
            try:
                if script.string:
                    ...
                    logger.debug(
                        f"Successfully parsed JSON-LD script: {len(script.string)} characters"
                    )
    ...
    @staticmethod
    def extract_links(soup: Any, base_url: str | None = None) -> dict[str, list[str]]:
        """
