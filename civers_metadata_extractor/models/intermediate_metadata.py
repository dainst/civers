...
class GeoLocation(BaseModel):
    ...
    geo_location_point: GeoLocationPoint | None = None
    geo_location_box: GeoLocationBox | None = None
...
class Date(BaseModel):
    ...
    date_format: str | None = None  # e.g., "YYYY-MM-DD", "YYYY", "YYYY/YYYY", "approximate"
...
class RelatedIdentifier(BaseModel):
    ...
    resource_type_general: ResourceTypeGeneral | None = None
...
class IntermediateMetadata(BaseMetadata):
    ...
    identifier: Identifier | None = None  # Auto-assigned by DataCite
    ...
    def validate_publication_year(cls, v):
        ...
        if v > current_year + 10:
            raise ValueError("Publication year cannot be more than 10 years in the future")
        return v
        ...
        return {
            "data": {
                ...
                "attributes": {
                    ...
                    "subjects": [self._format_subject(s) for s in self.subjects]
                    if self.subjects
                    else [],
                    "contributors": [self._format_contributor(c) for c in self.contributors]
                    if self.contributors
                    else [],
                    "dates": [self._format_date(d) for d in self.dates] if self.dates else [],
                    "language": self.language,
                    "alternateIdentifiers": [
                        self._format_alternate_identifier(ai) for ai in self.alternate_identifiers
                    ]
                    if self.alternate_identifiers
                    else [],
                    "relatedIdentifiers": [
                        self._format_related_identifier(ri) for ri in self.related_identifiers
                    ]
                    ...
                    "rightsList": [self._format_rights(r) for r in self.rights_list]
                    if self.rights_list
                    else [],
                    "descriptions": [self._format_description(d) for d in self.descriptions]
                    if self.descriptions
                    else [],
                    "geoLocations": [self._format_geolocation(g) for g in self.geo_locations]
                    if self.geo_locations
                    else [],
                    "fundingReferences": [
                        self._format_funding_reference(f) for f in self.funding_references
                    ]
                    if self.funding_references
                    else [],
                    "state": "draft",
                },
            }
    ...
    def _format_creator(self, creator: Creator) -> dict[str, Any]:
        ...
        result: dict[str, Any] = {"name": creator.creator_name, "nameType": creator.name_type.value}
    ...
    def _format_date(self, date: Date) -> dict[str, Any]:
        ...
        result = {"date": date.date, "dateType": date.date_type.value}
