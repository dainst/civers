...
from .models import ConfigDataModel, DomainConfig, KafkaConfig, TransportConfig
...
class ConfigValidationError(Exception):
    ...
    def __init__(
        self, message: str, validation_errors: list[str] | None = None, file_path: str | None = None
    ):
...
class ConfigValidator:
    ...
    def validate_config(
        self, config: ConfigDataModel, file_path: str | None = None
    ...
    def _validate_transport(self, transport: TransportConfig) -> None:
        ...
        if not transport.enabled:
            self.validation_errors.append(
                "No transports enabled - at least one transport is required"
            )
            return
    ...
    def _validate_mapper_consistency(self, domains: list[DomainConfig]) -> None:
        ...
        if len(mapped_common_fields) < len(common_datacite_fields) * 0.6:  # 60% threshold
            self.validation_warnings.append(
                f"Few common DataCite fields are mapped "
                f"({len(mapped_common_fields)}/{len(common_datacite_fields)}) - "
...
def validate_configuration_file(file_path: str, strict_mode: bool = True) -> ConfigDataModel:
    ...
    try:
        ...
        raise ConfigValidationError(
            f"Pydantic validation failed: {e}", validation_errors=[str(e)], file_path=file_path
        ) from e
    except Exception as e:
        raise ConfigValidationError(
            f"Configuration loading failed: {e}", file_path=file_path
        ) from e
