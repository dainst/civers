...
class YamlFileConfigLoader(ConfigLoaderInterface):
    ...
    def load(self) -> ConfigDataModel:
        ...
        try:
            ...
            error_msg = "Invalid YAML syntax in configuration file"
            if hasattr(e, "problem_mark") and e.problem_mark:
                error_msg += (
                    f" at line {e.problem_mark.line + 1}, column {e.problem_mark.column + 1}"
                )

            raise ConfigurationError(error_msg, file_path=self.file_path, original_error=e) from e
    ...
    def _validate_file_access(self) -> None:
        ...
        if not os.path.isfile(self.file_path):
            raise ConfigurationError(
                f"Configuration path is not a file: {self.file_path}", file_path=self.file_path
            )
