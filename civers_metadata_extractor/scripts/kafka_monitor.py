...
class MetadataExtractionStatusMonitor:
        ...
        try:
            topics = [
                ...
                self.topics.get(
                    ...
                    self.topics.get("metadata_extracted", "metadata.extracted"),
                ),
    ...
    async def start_monitoring(self):
        ...
        logger.info(
            "📊 Monitoring for Kafka messages with native async iteration (Press Ctrl+C to stop)..."
        )
        try:
            ...
            async for message in self.consumer:
                ...
                try:
                    ...
                    if topic == self.topics.get("metadata_status", "metadata.status"):
                        await self.handle_status_event(
                            value, key, message.partition, message.offset
                        )
                    elif topic == self.topics.get(
                        "metadata_extraction_completed",
                        self.topics.get("metadata_extracted", "metadata.extracted"),
                    ):
                        await self.handle_completed_event(
                            value, key, message.partition, message.offset
                        )
                    elif topic == self.topics["metadata_extraction_failed"]:
                        await self.handle_failed_event(
                            value, key, message.partition, message.offset
                        )
                    elif topic == self.topics.get("metadata_quality"):
                        await self.handle_quality_event(
                            value, key, message.partition, message.offset
                        )
    ...
    async def _print_stats(self):
        ...
        logger.info(
            f"📈 Stats: Active={active_count}, Completed={self.stats['requests_completed']}, "
...
async def async_main():
    ...
    monitor = MetadataExtractionStatusMonitor()
