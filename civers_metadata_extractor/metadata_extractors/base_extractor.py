...
class ExtractionResult:
    ...
    raw_data: Dict[str, Any]  # Raw extracted data before mapping
    extraction_type: str  # Type of extraction used (jsonld, meta_tags, css_selectors)
    source_url: str
    processing_time_seconds: float
    warnings: List[str] = None  # Non-fatal issues during extraction
...
class BaseExtractor(ABC):
    ...
    def __init__(self, name: str):
        ...
        Args:
            name: Human-readable name for this extractor type
        """
        self.name = name
    ...
    def can_extract(self, html_content: str) -> bool:
        """
        Check if this extractor can process the given HTML content.
        
        This method should quickly determine if the HTML contains the type
        of metadata this extractor is designed to handle.
        
        Args:
            html_content: The HTML content to check
            
        Returns:
            True if this extractor can process the content, False otherwise
        """
        pass
    ...
    def get_priority(self) -> int:
        ...
        can handle the same content. Override in subclasses as needed.
        
        Returns:
            Priority value (higher = more preferred). Default: 0
        """
        return 0
    ...
    def __repr__(self) -> str:
        ...
        return f"{self.__class__.__name__}(name='{self.name}', type='{self.get_extractor_type()}')"
