...
from metadata_extraction_services.metadata_extraction_service_interface import (
...
class KafkaTransportService(TransportServiceInterface):
    ...
    def __init__(
        self, config: ConfigDataModel, metadata_service: MetadataExtractionServiceInterface
    ):
        self.config = config
    ...
    async def start(self):
        ...
        try:
            ...
            request_topic = self.topics.get("metadata_extraction_requests") or self.topics.get(
                "metadata_requests"
            )
            if request_topic:
                self.register_handler(request_topic, self._handle_metadata_extraction_request)
            else:
    ...
    def _deserialize_message_value(self, message, topic: str):
        ...
        try:
            ...
            if isinstance(message.value, bytes):
                try:
                    ...
                    if not isinstance(message_data, dict):
                        logger.error(
                            f"❌ Message from {topic} is not a JSON object, got {type(message_data)}"
                        )
                        logger.debug(f"   Raw value: {message.value[:200]!r}...")
                        return None
                    ...
                    logger.debug(f"   Raw message value: {message.value[:200]!r}...")
                    return None
            ...
            # Unexpected value type
                ...
                logger.error(
                    f"❌ Unexpected message value type from {topic}: {type(message.value)}"
                )
    ...
    async def _handle_metadata_extraction_request(self, message, message_data: dict[str, Any]):
        ...
        try:
            ...
            logger.info(
                f"🚀 Processing request {extraction_request.request_id}: {extraction_request.url}"
            )
            ...
            if result.success:
                ...
                completion_event = MetadataExtractionCompletedEvent(
                    request_id=extraction_request.request_id,
                    url=extraction_request.url,
                    extracted_metadata=result.intermediate_metadata.model_dump()
                    ...
                    metadata_statistics={
                        "total_fields": len(result.intermediate_metadata.model_dump())
                        if result.intermediate_metadata
                        else 0
                    },
                    quality_assessment={"extraction_successful": True}
                    if result.intermediate_metadata
                    else None,
                    extraction_details={},
                    workflow_stages_completed=["received", "metadata_extraction", "completed"],
                    total_fields_extracted=len(result.intermediate_metadata.model_dump())
                    if result.intermediate_metadata
                    else 0,
                )
                ...
                logger.info(
                    f"✅ Request {extraction_request.request_id} completed successfully in {completion_event.processing_time_seconds:.2f}s"
                )
            else:
                # Extraction failed
    ...
    async def publish_metadata_extraction_request(
        self, event: MetadataExtractionRequestEvent
    ) -> bool:
        """Publish a metadata extraction request event to Kafka."""
        return await self.event_publisher.publish_event(
    ...
    async def send_response(self, destination: str, message: dict[str, Any], **kwargs) -> bool:
        ...
        try:
            ...
            return await self.event_publisher.publish_event(destination, key, message)
        except Exception as e:
    ...
    def get_transport_info(self) -> dict[str, Any]:
        ...
        return {
            ...
            "kafka_config": {
                ...
                "consumer_group": self.kafka_config.consumer_group if self.kafka_config else None,
                "topics": self.topics,
            },
            "capabilities": {
