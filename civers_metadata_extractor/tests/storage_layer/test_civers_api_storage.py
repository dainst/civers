...
import pytest
import json
from storage_layer.civers_rest_api_storage_strategy import (
    CiversRestApiStorageStrategy,
    HTTPX_AVAILABLE
)
from storage_layer.storage_strategy import StorageResult

# Skip all tests if httpx is not available
pytestmark = pytest.mark.skipif(
    not HTTPX_AVAILABLE,
    reason="httpx library not installed (install with: uv sync --extra http)"
)
...
def test_import_error_when_httpx_not_available():
    """Test that proper error is raised when httpx is not installed."""
    # This test only makes sense if httpx is actually not available
    # Since we're skipping all tests when httpx is missing, this is more
    # of a documentation of expected behavior
    if not HTTPX_AVAILABLE:
        with pytest.raises(ImportError, match="httpx library is required"):
            CiversRestApiStorageStrategy(upload_url="http://test.com")
