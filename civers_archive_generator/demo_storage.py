...
async def main():
    ...
    if not args.with_api and not args.use_config:
        print(f"\n💡 Options:")
        print(f"   --with-api        Test with hardcoded API URL")
        print(f"   --use-config      Use config from configs/data/")
        print(f"   --artifacts       Test all artifact types")
